...
- The default SQLite connection string is still `Data Source=KicsitLibrary.db`, resolved from `AppContext.BaseDirectory`. This is predictable but- **UI Enhancements Needed**: As documented in `PHASE 13A UI POLISH AUDIT.md`, several UI screens lack consistent margin spacing, modern vector icons, and polished DataGrid aesthetics.
