...
- The default SQLite connection string is still `Data Source=KicsitLibrary.db`, resolved from `AppContext.BaseDirectory`. This is predictable but- **UI Enhancements Completed/Limitations**: Phase 13B Professional UI Polish implemented margin spacing, modern vector icons (via emojis), and polished DataGrid aesthetics. Advanced animations and glassmorphism were avoided per administrative theme rules. Payment options in Receive Material remain a simple ComboBox to preserve business logic.
- **ClickOnce Installer Certificate**: Deployment packaging currently creates unsigned Portable publish targets; a university-issued code signing certificate is required before production ClickOnce distribution.
- **Cloud Sync (Supabase) Unimplemented**: Local SQLite database changes are not yet synchronized to remote Postgres servers for offsite recovery. Existing databases rely on `EnsureCreatedAsync` plus additive SQLite compatibility SQL, which is not a full production migration strategy.
